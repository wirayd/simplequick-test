describe("Ext.grid.column.Action", function(){
    ...
    describe("stopSelection", function() {
        it("should not select the row or focus the cell when clicking the action with stopSelection: true", function() {
            makeGrid({
                columns: [{}, {
                    xtype: 'actioncolumn',
                    stopSelection: true,
            ...
            triggerAction();
            expect(grid.getSelectionModel().isSelected(store.first())).toBe(false);
            var pos = grid.view.getNavigationModel().getPosition();
            expect(pos.rowIdx).toBe(0);
        ...
        describe("enabled/disabled state", function() {
            it("should not fire the handler if configured as disabled", function() {
                ...
                var view   = grid.getView(),
                    ...
                    img    = Ext.get(rowEl).down('img.x-action-col-icon'),
